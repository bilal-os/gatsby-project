    ...
    <div className="pl-3">
      {social.map(({ name, url }, i) => {
        ...
        return (
          <a
            aria-label={name}
            className={`inline-flex w-12 h-12 justify-center items-center rounded-full ${colorsClass} hover:shadow-lg transition-shadow duration-150`}
            href={url}
            key={name}
            rel="noopener noreferrer"
            target="_blank"
            style={{ marginRight: "10px" }} // Add space with margin
          >
